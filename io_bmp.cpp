...
static void to_little_endian(io_int32 *words, int num_words){
    io_int32 test = 1;
...
static inline void from_little_endian(io_int32 *words, int num_words){
...
int bmp_in__open(bmp_in *state, const char *fname){
    ...
    state->alignment_bytes = (4-state->line_bytes) & 3; // Pad to a multiple of 4 bytes
    return 0;
}
...
int bmp_in__get_line(bmp_in *state, io_byte *line){
    ...
    if (fread(line,1,(size_t) state->line_bytes,state->in) != (size_t) state->line_bytes){
        return(IO_ERR_FILE_TRUNC);
    }
...
int bmp_out__open(bmp_out *state, const char *fname, int width, int height, int num_components){
    ...
    int file_bytes = header_bytes + (state->line_bytes+state->alignment_bytes)*state->rows;
    magic[0] = 'B'; magic[1] = 'M';
...
int bmp_out__put_line(bmp_out *state, io_byte *line){
    ...
    if (fwrite(line,1,(size_t) state->line_bytes,state->out) != (size_t) state->line_bytes){
